--==========================================================
-- ENEMY TRACKER FINAL POLISH v18
--
-- CHANGES
...
-- 6) MAP OFF hides parts/effects/water but preserves players + medkits.
--
-- LocalScript
...
-- Desired underground tracking height.
-- Roblox usually deletes falling character parts near
-- Workspace.FallenPartsDestroyHeight, so a safety clamp is used.
local TRACK_DESIRED_Y = -495
local FALL_DESTROY_SAFETY_MARGIN = 10
...
local function teleportToManualItem(itemKey, displayName)
	...
	-- One-shot TP only. No continuous item tracking.
	myRoot.CFrame =
...
main.Position = UDim2.new(0, 18, 0.5, -233)
...
subtitle.Size = UDim2.new(1, -62, 0, 15)
subtitle.Position = UDim2.new(0, 14, 0, 29)
subtitle.BackgroundTransparency = 1
subtitle.Text = "MOBILE - TRACK / ITEMS / MEDKIT"
...
collapseButton.Size = UDim2.new(0, 34, 0, 34)
collapseButton.Position = UDim2.new(1, -42, 0, 9)
...
sliderLabel.Position = UDim2.new(0, 10, 0, 252)
sliderLabel.BackgroundTransparency = 1
sliderLabel.Text = "TRACK Y : SAFE -495" .. " studs"
...
sliderTrack.Position = UDim2.new(0, 10, 0, 274)
...
listTitle.Size = UDim2.new(1, -20, 0, 17)
listTitle.Position = UDim2.new(0, 12, 0, 292)
...
list.Size = UDim2.new(1, -20, 0, 82)
list.Position = UDim2.new(0, 10, 0, 313)
...
local function getSafeTrackingY()
	local desiredY = TRACK_DESIRED_Y

	local enabled = true
	local destroyY = -500

	pcall(function()
		enabled = workspace.FallHeightEnabled
	end)

	pcall(function()
		destroyY = workspace.FallenPartsDestroyHeight
	end)

	-- If Roblox fall destruction is disabled, use exactly -495.
	if enabled == false then
		return desiredY
	end

	-- Keep the character assembly safely above the destruction line.
	local minimumSafeY =
		destroyY + FALL_DESTROY_SAFETY_MARGIN
...
RunService.RenderStepped:Connect(function()
	...
	-- Y aims for -495 but is clamped above the actual
	-- FallenPartsDestroyHeight when necessary.
	...
	-- Follow only the target's X/Z.
	-- Stay lying down at the fixed world Y coordinate.
	local safeTrackY = getSafeTrackingY()
...
local expandedSize = UDim2.new(0, 320, 0, 466)
local collapsedSize = UDim2.new(0, 320, 0, 52)
